...
export default function NoticeEditPage() {
  ...
  const [formData, setFormData] = useState<NoticeFormData | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData || !formData.title.trim() || !formData.content.trim()) return;
    setSaving(true);
    try {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-2xl font-semibold">{noticeLabels.editTitle}</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="max-w-2xl space-y-6">
        <div className="space-y-1.5">
          ...
          <Input
            ...
            onChange={(e) => setFormData((prev) => prev && ({ ...prev, title: e.target.value }))}
        ...
        <div className="space-y-1.5">
          ...
          <Textarea
            ...
            onChange={(e) => setFormData((prev) => prev && ({ ...prev, content: e.target.value }))}
        ...
        <div className="flex items-center gap-3">
          <Switch
            ...
            onCheckedChange={(checked: boolean) =>
              setFormData((prev) => prev && ({ ...prev, isPinned: checked }))
            }
          />
